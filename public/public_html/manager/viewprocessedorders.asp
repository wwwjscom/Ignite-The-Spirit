	...
	<tr>
		...
		<td width=636 valign=top bgcolor=ffcc00>
			...
			<table border=0 cellpadding=0 cellspacing=0>
				<tr>
					<td width=110>
						<span class=normg>Order Number</span><br>
					</td>
					<td width=170>
			...
			<%
			pagetot = 0
			sql = "select customernum, ship_first_name, ship_last_name, date, amount_merchandise, amount_shipping from fire_customers where status = 1 order by customernum"
			rs.open sql
			while not rs.eof
				tot = 0
				...
				pagetot = pagetot + tot
				response.write "<tr><td><span class=norm><a href=vieworder.asp?customernum=" & rs("customernum") & ">" & rs("customernum") & "</span><br></td>"
				response.write "<td><span class=norm><a href=vieworder.asp?customernum=" & rs("customernum") & ">" & rs("date") & "</span><br></td>"
				...
				response.write "<tr><td colspan=4><img src=lilshimmy.gif height=3><br></td></tr>"
				rs.movenext
			wend
			%>
				<tr>
